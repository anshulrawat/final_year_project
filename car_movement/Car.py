...
class Car:
    ...
    def is_collision(self):
        if(self.rect.x+self.move_x <= 0 or self.rect.x+self.move_x >= 1000 or self.rect.y+self.move_y <= 0 or self.rect.y+self.move_y >= 600):
            return True
        elif(main_s.get_at((int(self.rect.x+self.move_x),int(self.rect.y+self.move_y)))!=surface):
             return True
        else:
            return False
